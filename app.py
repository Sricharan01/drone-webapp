...
cfg_model_path = 'models/yolov5L_custom.pt'
model = None
confidence = .25
...
def video_input(data_src):
    vid_file = None
    if data_src == 'Sample data':
        vid_file = "data/sample_videos/video_drone2.mp4"
    else:
...
def main():
    ...
    if not os.path.isfile(cfg_model_path):
        ...
        # input options
        input_option = st.sidebar.selectbox("Select input type: ", ['image', 'video'])

        # input src option
        data_src = st.sidebar.selectbox("Select input source: ", ['Sample data', 'Upload your own data'])
